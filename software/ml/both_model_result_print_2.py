...
# Shared variables for class probabilities
probs_gru = None
probs_lgbm = None
...
try:
    fusion_model = joblib.load(model_path('best_meta_learner_model.pkl'))
    print("Fusion model loaded successfully.")
except FileNotFoundError:
    print("Fusion model file not found. Make sure 'best_meta_learner_model.pkl' exists.")
    fusion_model = None  # Set to None or handle appropriately
...
def feature_extraction():
    ...
    while True:
        ...
        with prediction_lock:
            global prediction_lgbm, features_lgbm, probs_lgbm
            ...
            probs_lgbm = prediction  # Shape: (1, num_classes)
...
try:
    ...
    input_size = 3
    hidden_size = 64  # Adjusted to 64 to match the expected number of features
    num_layers = 2
    dropout_rate = 0.5
...
gesture_labels = [
    'Hand Waving While Walking',
    'Hand Waving While Sitting',
    'Hand Waving While Standing',
    'Hand Stationary While Sitting',
    'Hand Stationary While Walking',
    'Hand Stationary While Standing',
    'Hand Stationary While Sleeping',
    'Hand Rising While Standing',
    'Hand Rising While Sitting',
    'Standing Up',
    'Sitting Down',
    'Lying Down',
    'Waking Up',
...
def data_processor_gru():
    ...
    while True:
        ...
        if len(recent_data) >= 2:
            ...
            # Ensure the sequence length is consistent (e.g., pad or truncate to specific length)
            # For simplicity, let's truncate or pad to length 64
            sequence_length = 64
            if len(sequence_data) >= sequence_length:
                sequence_data = sequence_data[-sequence_length:]  # Take the last 64 samples
            else:
                # Pad with zeros at the beginning
                padding = np.zeros((sequence_length - len(sequence_data), 3))
...
def process_and_print_predictions():
    """Function to merge features and probabilities from both models, apply the fusion model, and print predictions."""
    sequence_counter = 0  # Sequence ID counter
    prediction_history = deque(maxlen=3)  # Stores the last 4 predictions for smoothing
    while True:
        time.sleep(0.3)
        with prediction_lock:
            ...
            probs_gru_local = probs_gru
            probs_lgbm_local = probs_lgbm

        # Existing processing and printing with features
        if feats_gru is not None and feats_lgbm is not None and probs_gru_local is not None and probs_lgbm_local is not None:
            ...
            # Adjust the feats_gru to have the expected number of features
            num_feats_gru = feats_gru.shape[1]  # Should be hidden_size
            num_feats_lgbm = feats_lgbm.shape[1]  # Should be 18

            # Create headers for raw features (0_raw to {num_feats_gru-1}_raw)
            headers_raw = ['{}_raw'.format(i) for i in range(num_feats_gru)]
            # Create headers for feature branch (0_feature to {num_feats_lgbm-1}_feature)
            headers_feature = ['{}_feature'.format(i) for i in range(num_feats_lgbm)]

            # Concatenate features
            merged_features = np.concatenate([feats_gru, feats_lgbm], axis=1)  # Shape: (1, total_features)

            # Create DataFrame for features
            df_merged_features = pd.DataFrame(merged_features, columns=headers_raw + headers_feature)
            df_merged_features['sequence'] = sequence_counter
            df_merged_features['label'] = 'N/A'  # Or None or predicted label if available

            # Rearrange columns to match the order specified
            df_merged_features = df_merged_features[headers_raw + ['sequence', 'label'] + headers_feature]

            # Now, process and prepare merged probabilities
            num_classes = probs_gru_local.shape[1]
            # Create columns for GRU probabilities
            probs_gru_df = pd.DataFrame(probs_gru_local, columns=[f'class_{i}_raw' for i in range(num_classes)])
            # Create columns for LightGBM probabilities
            probs_lgbm_df = pd.DataFrame(probs_lgbm_local, columns=[f'class_{i}_feature' for i in range(num_classes)])
            # Add sequence and label columns
            probs_df = pd.concat([probs_gru_df, probs_lgbm_df], axis=1)
            probs_df['sequence'] = sequence_counter
            probs_df['label'] = 'N/A'  # or None

            # Rearrange columns to match the order in merged_test_predictions.csv
            cols = [f'class_{i}_raw' for i in range(num_classes)] + ['sequence', 'label'] + [f'class_{i}_feature' for i in range(num_classes)]
            probs_df = probs_df[cols]

            # Combine features and probabilities for fusion model input
            X_features = df_merged_features.drop(columns=['sequence', 'label']).values
            X_preds = probs_df.filter(regex='class_.*').values
            X_combined = np.hstack([X_features, X_preds])

            # Apply fusion model
            if fusion_model is not None:
                y_pred = fusion_model.predict(X_combined)
                final_prediction = y_pred[0]  # Assuming batch size is 1
                # Map the prediction back to gesture labels if available
                final_gesture_label = gesture_labels[final_prediction] if final_prediction < len(gesture_labels) else 'Unknown'
            else:
                final_prediction = 'N/A'
                final_gesture_label = 'N/A'
            ...
            try:
                most_common_prediction = max(set(prediction_history), key=prediction_history.count)
                # Map the prediction back to gesture labels if available
                final_gesture_label_smoothed = gesture_labels[most_common_prediction] if most_common_prediction < len(gesture_labels) else 'Unknown'
            except ValueError:
                # In case prediction_history is empty
                most_common_prediction = 'N/A'
                final_gesture_label_smoothed = 'N/A'

            # Print the final smoothed prediction from the fusion model
            print(f"\nFinal Smoothed Prediction from Fusion Model: {most_common_prediction} ({final_gesture_label_smoothed})")
            ...
            # Also, set 'final_prediction' to 'N/A'
            final_prediction = 'N/A'
            final_gesture_label = 'N/A'
